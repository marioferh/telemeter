...
# If no arguments are passed an integration test scenario is run.
...
( ./_output/bin/prometheus --config.file=./test/prom-local.conf --web.listen-address=localhost:9090 "--storage.tsdb.path=$(mktemp -d)" --log.level=warn ) &
...
(
./_output/bin/thanos query \
    --grpc-address=127.0.0.1:9007 \
    --http-address=127.0.0.1:9008 \
    --store=127.0.0.1:9006
) &

#Wait for all components correct state
sleep 30

retries=100
while true; do
 echo "check succesfuly retrieve metrics from local prometheus server"
  if [[ "${retries}" -lt 0 ]]; then
    echo "error: Did not successfully retrieve cluster metrics from the local Prometheus server" 1>&2
    exit 1
  fi
  echo "verify we scrape metrics from the test cluster and give it _id test"
  ...
  echo "verify we rename scrape_samples_scraped to scraped"
  ...
  echo "verify we got alerts as remapped from ALERTS"
  ...
  echo "verify we don't get elided labels"
